...
/*--------------------------------------------------------------------------------------------*\
  2010/12/11  Ron Shuff Jr. - modified errors to be array of Class ErrorMessage (defined at end of this page)
  2010/12/27  Ron Shuff Jr. - added file_required expecting $_FILES to be submitted
  2011/07/04  Ron Shuff Jr. - changed rule separator to | instead of ,
  2011/07/04  Ron Shuff Jr. - added regex3 for separate phone fields
  2011/07/05  Ron Shuff Jr. - added valid_datetime and valid_datetimenullable
  2011/09/25  Ron Shuff Jr. - added money
  2011/09/25  Ron Shuff Jr. - added boolean 
  2011/09/27  Ron Shuff Jr.	- added answer_list
  ...
  Parameters: field - the POST / GET fields from a form which need to be validated.
   ...
   "[if:FIELDNAME=VALUE,]REQUIREMENT,fieldname[,fieldname2 [,fieldname3, date_flag]],error message"
              ...
              Valid REQUIREMENT strings are: 
                ...
                "money"  - field must contain digits . , and $ only.  
                "boolean"  - following values only (1,0,true,false,True,False,TRUE,FALSE,yes,no,YES,NOYes,No,T,F,t,f,Y,N,y,n) 
                ...
                "reg_exp3"      - field must match the supplied regular expression.  
                      fieldname:  the name of the 1st field
                      fieldname2:  the name of the 2nd field
                      fieldname3:  the name of the 3rd field
                      fieldname4: the regular expression
                      fieldname5: (optional) flags for the reg exp (like i for case insensitive
                "letters_only" - field must only contains letters (a-Z)
                "answer_list" - field must only one of the answers in the list
                	fieldname: the name of the field
                	fieldname2: a |(pipe) delimited list of possible answers
                ...
                "valid_datetime"  - field has to be a valid date
                "valid_datetimenullable"  - field has to be a valid date or null
...
{ 
  ...
  for ($i=0; $i<count($rules); $i++)
  {
    ...
    if (count($row) == 6)        // valid_date or reg_exp3
    {
      $field_name2   = $row[2];
      $field_name3   = $row[3];
      $date_flag     = $row[4];
    ...
    {
	  case "file_required":
        $file = $fields[$field_name];
        if($file['error'] > 0 || $file['size'] <= 0)
        {
        	if($file['error'] == 1) { $error_message .= "(FILE SIZE EXCEEDED)"; }
            if($file['error'] == 2) { $error_message .= "(FILE SIZE EXCEEDED)"; }
            if($file['error'] == 3) { $error_message .= "(PARTIALLY UPLOADED)"; }
            if($file['error'] == 4) { $error_message .= "(NO FILE)"; }
            if($file['error'] == 7) { $error_message .= "(FAILED TO WRITE)"; }
            $errors[] = new ErrorMessage($field_name, $error_message);
        }
        break;
      case "required":
      ...
      case "money":
		$regexp="/^(?:\d+)(?:,\d{3})*(?:\.\d{2})?$/";
        if (isset($fields[$field_name]) && !(empty($fields[$field_name]) && !is_numeric($fields[$field_name])) && !preg_match($regexp, $fields[$field_name]))
          $errors[] = new ErrorMessage($field_name, $error_message);
        break;

      case "boolean":
		$regexp="/^([10TFYNtfyn]|True|False|TRUE|FALSE|true|false|Yes|No|YES|NO|yes|no)$/";
        if (isset($fields[$field_name]) && !(empty($fields[$field_name]) && !is_numeric($fields[$field_name])) && !preg_match($regexp, $fields[$field_name]))
      	$errors[] = new ErrorMessage($field_name, $error_message);
      	break;
      ...
      case "answer_list":
		$list = explode("|", $field_name2);
		$listHasError = true;
		foreach($list as $item)
		{
			if(isset($fields[$field_name]) && $item == $fields[$field_name])
			{
				$listHasError = false;
				break;
			}
      ...
      case "valid_datetime":
      	if(strtotime($fields[$field_name]) === false)
      	{
      		$errors[] = new ErrorMessage($field_name, $error_message);
      	}
      	break;
      	
      case "valid_datetimenullable":
      ...
      case "custom_alpha":
        ...
        if (!(empty($fields[$field_name]) && !is_numeric($fields[$field_name])) && !preg_match("/$reg_exp_str/", $fields[$field_name]))
          $errors[] = new ErrorMessage($field_name, $error_message);
        break;
      ...
      case "reg_exp":
        ...
        if (!(empty($fields[$field_name]) && !is_numeric($fields[$field_name])) && !preg_match($reg_exp, $fields[$field_name]))
        {
        	$errors[] = new ErrorMessage($field_name, $error_message);
        }
        break;
      ...
      case "reg_exp3":
        $reg_exp_str = $date_flag;
        $reg_exp = "/" . $reg_exp_str . "/";
        if (!(empty($fields[$field_name]) && !is_numeric($fields[$field_name])) && !preg_match($reg_exp, $fields[$field_name] . $fields[$field_name2] . $fields[$field_name3]))
        {
	        $errors[] = new ErrorMessage($field_name, $error_message);
        }
        break;
